...
int do_start(int syscall, int pid, int status) {
	...
	if (pid == -1)
		pid=getpid();
	a = vsyscall_arg(MY_CUSTOM_SYSCALL, 3, REQUEST_START_MONITORING, syscall, pid);

	if (pid == 0)
	{
		printf("syscall%i,EINVAL:%i,EPERM:%i, EBUSY:%i,,a:%i",syscall,a == -EINVAL,a == -EPERM,a == -EBUSY, a);
...
int do_stop(int syscall, int pid, int status) {
	int a;
	a = vsyscall_arg(MY_CUSTOM_SYSCALL, 3, REQUEST_STOP_MONITORING, syscall, pid);
