...
int do_start(int syscall, int pid, int status) {
	...
	if (pid == -1)
		pid=getpid();
	int a = vsyscall_arg(MY_CUSTOM_SYSCALL, 3, REQUEST_START_MONITORING, syscall, pid);
	//printf("EV%i.EP%i,EB%i.%i.", a == -EINVAL, a == -EPERM, a == -EBUSY,a);

	test("%d start", syscall, a == status);
	return 0;
}

int do_stop(int syscall, int pid, int status) {
	int a =vsyscall_arg(MY_CUSTOM_SYSCALL, 3, REQUEST_STOP_MONITORING, syscall, pid);
		//printf("EV%i.EP%i,EB%i.%i.", a == -EINVAL, a == -EPERM, a == -EBUSY,a);
	test("%d stop", syscall,a == status);
	return 0;
}
...
void test_syscall(int syscall) {
	...
	do_stop(syscall, 1, 0);
	printf("C!!!");
	do_start(syscall, 9, -EBUSY);
	printf("C");
	do_stop(syscall, 0, 0);
	
	printf("D");
	do_start(syscall, 10, 0);
	printf("D");
	do_start(syscall, 11, 0);
	printf("D");
	do_start(syscall, 0, 0);
	printf("D");
	do_stop(syscall, 0, 0);
